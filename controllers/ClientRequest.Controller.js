...
exports.create = (req, res) => {
  ...
  // }
  console.log(req.body, "clientRequest");
...
exports.update = (req, res) => {
  ...
  ClientRequest.findById(req.body._id).then((user) => {
    if (user) {
      ...
      //   if (req.body.eventDate) user.eventDate = req.body.eventDate;
      //   if (req.body.eventCity) user.eventCity = req.body.eventCity;
      //   if (req.body.maxBudget) user.maxBudget = req.body.maxBudget;
      ...
      user.save((err) => {
        if (err) {
          ...
          res.status(200).json("Clent Request updated successfully");
        }
      });
    } else {
...
exports.setStatusLeadsVendor = (req, res) => {
  ClientRequest.findOne({
    clientEmail: req.body.email,
    eventDate: req.body.date,
  }).then((user) => {
    if (user) {
      if (user.reqStatus == "allow") user.reqStatus = "false";
      else user.reqStatus = "allow";
      user.save((err) => {
        if (err) {
          res.status(500).json(err);
        } else {
...
exports.delLeadVendor = (req, res) => {
  ClientRequest.findOneAndRemove(
...
exports.delLeadVendorId = (req, res) => {
  ClientRequest.findOneAndRemove({ vendorId: req.body.id }, function (
    err,
    business
  ) {
    if (err) res.status(401).json(err);
