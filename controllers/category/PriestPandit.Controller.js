...
const PriestPandit = mongoose.model('PriestPandit');

exports.create = (req, res) => {
    // var role = req.type;
    // const car = new Car(req.body);
    PriestPandit.findOne({ email: req.body.email }).then(bridal => {
        if (bridal) {
            return res.status(400).json({ email: 'bridal designer already exists' });
        } else {
            const bridal = new PriestPandit(req.body);
...
exports.update = (req, res) => {
    // role = req.type;
    userId = req.body._id;
    PriestPandit.findById(userId, function (err, car) {
        ...
        car.samagriIncluded = req.body.samagriIncluded == '' ? false : true;
        if (req.body.preWeddingRokaPujaHawan)
            car.serviceOffered.preWeddingRokaPujaHawan = req.body.preWeddingRokaPujaHawan;
        if (req.body.saganPujaHawan)
            car.serviceOffered.saganPujaHawan = req.body.saganPujaHawan;
        if (req.body.weddingPujaHawan)
            car.serviceOffered.weddingPujaHawan = req.body.weddingPujaHawan;
        if (req.body.matchingKundlis)
            car.serviceOffered.matchingKundlis = req.body.matchingKundlis;
        if (req.body.astrology)
            car.serviceOffered.astrology = req.body.astrology;
